...
export default function CompetitionDayContent({ competitions }) {
  ...
  const { data: fetchedMatches, refetch: refetchMatches } = useQuery(
    ...
    async () => {
      const res = await fetch(
        `/api/match?competitionId=${selectedCompetition?.id}`
      );
      return res.json();
    },
    {
  ...
  const customSortMatches = useMemo(() => {
    return (matches) => {
      return matches.sort((a, b) => {
        ...
        return (
          Math.floor(a.fightNumber / 100) - Math.floor(b.fightNumber / 100)
        );
      });
    };
  }, []);
  ...
  useEffect(() => {
    const ongoingMatchIds = Object.values(ongoingMatches)
      .flat()
      .map((match) => match.id);
            ...
            <ListItem button key={area} onClick={() => handleAreaChange(area)}>
              ...
              <Box key={area}>
                <TextField
                  ...
                  onChange={(e) =>
                    handleCurrentFightChange(area, parseInt(e.target.value))
                  }
                  sx={{ margin: 1 }}
                />
                <Typography
                  sx={{
                    color: duplicateFights.has(remainingFightsByArea[area])
                      ? "red"
                      : "inherit",
                  }}
                >
                  Remaining Fights in Area {area}: {remainingFightsByArea[area]}{" "}
                  fights
                </Typography>
              </Box>
            </ListItem>
      ...
      <Box flex={1} p={2}>
        ...
        <Select
          ...
          renderValue={(selected) =>
            selected ? selectedCompetition.name : "Select a competition"
          }
          sx={{ marginBottom: 2 }}
        >
          {comp &&
            comp.map((competition) => (
              <MenuItem
                key={competition.id}
            ...
            filteredMatches.map((match) => (
              ...
              >
                ...
                <Box sx={{ flexGrow: 1, textAlign: "left" }}>
                  {" "}
                  {/* Box contenant le texte pour le contrôle du layout */}
                  <Typography variant="h6" sx={{ fontSize: "1.5rem" }}>
                    {match.fighter.firstName} {match.fighter.lastName}
                  </Typography>
                  <Typography variant="body1" gutterBottom>
                    Rounds Recorded:{" "}
                    {roundSavedData[match.id]
                      ? roundSavedData[match.id].length
                      : 0}
                  </Typography>
                </Box>
                <Button
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center" }}>
                  ...
                  <Box sx={{ flexGrow: 1 }}>
                    <Typography variant="h6" gutterBottom>
                      Fight #{match.fightNumber} - {match.fighter.firstName}{" "}
                      {match.fighter.lastName}
                    </Typography>
                ...
                {Array.isArray(roundData[match.id]) ? (
                  roundData[match.id].map((round) => (
                    <Box key={round.id} sx={{ paddingLeft: 2, paddingTop: 1 }}>
                      <Typography variant="body2">
                        Round Score: Blue {round.scoreBlue} - Red{" "}
                        {round.scoreRed}
                      </Typography>
                      <Typography variant="body2">
                  ...
                  <Box sx={{ paddingLeft: 2, paddingTop: 1 }}>
                    <Typography variant="body2">
                      Round Score: Blue {roundData[match.id]?.scoreBlue} - Red{" "}
                      {roundData[match.id]?.scoreRed}
                    </Typography>
                    <Typography variant="body2">
                    ...
                    <Typography variant="body2">
                      Result:{" "}
                      {roundData[match.id]?.isWinner ? "Winner" : "Loser"}
                    </Typography>
                  </Box>
                )}
              </Paper>
